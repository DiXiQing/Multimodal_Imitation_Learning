...
class MultimodalGripper(nn.Module):
    """Camera(ResNet18) + EMG(1D CNN) → 角度回归"""
    def __init__(self):
        super().__init__()

        # Camera分支：ResNet18特征提取
        resnet = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        self.image_encoder = nn.Sequential(*list(resnet.children())[:-1])  # → (B, 512, 1, 1)

        # EMG分支
        self.emg_encoder = EMGEncoder()  # → (B, 32)
    ...
    def forward(self, img, emg):
        # Camera特征
        img_feat = self.image_encoder(img)       # (B, 512, 1, 1)
        img_feat = img_feat.view(img_feat.size(0), -1)  # (B, 512)

        # EMG特征
        emg_feat = self.emg_encoder(emg)         # (B, 32)

        # 拼接融合
        fused = torch.cat([img_feat, emg_feat], dim=1)  # (B, 544)
